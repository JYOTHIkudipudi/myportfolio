...
const frontendAndBackend = [
    ...
    { name: "Spring Boot", icon: "bx bx-leaf" },
];
...
const softSkills = [
    ...
    { name: " Communication", icon: "bx bx-message-rounded" },
    { name: "Time Management", icon: "bx bx-timer" },
    { name: "Adaptability", icon: "bx bx-sync" },
    { name: "Logical Thinking", icon: "bx bx-brain" },
];
    ...
    <ul className="list-none mt-4 flex flex-wrap gap-3 justify-center md:justify-start">
        {items.map((item, index) => {
            ...
            return (
                <motion.li
                    key={name}
                    className="flex items-center gap-3 hover:cursor-pointer bg-white dark:bg-neutral-900 rounded-full px-6 py-6 shadow-md text-neutral-900 dark:text-white font-semibold font-mulish transition-colors duration-300 min-w-[120px] max-w-fit justify-center text-center"
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    whileHover={{
                        color: "#3c62fbff",
                        scale: 1.05,
                        backgroundColor: "rgba(68, 119, 187, 0.1)",
                ...
                >
                    {icon ? (
                        <i className={`${icon} text-blue-400 text-2xl`}></i>
                    ) : (
                ...
                <div className="flex flex-col gap-12 pt-16 px-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <span className="bg-blue-400 dark:bg-blue-400 h-6 w-[0.5rem] rounded-sm"></span>
